...
int main(int argc, char *argv[]) {
    ...
    for (std::filesystem::directory_entry entry : std::filesystem::directory_iterator(folder)) {
        ...
        if (!shader.parse(fileData.data(), fileData.size())) {
            ERR_PRINT("Failed to parse SPIR-V data from %s.\n", spirvPathU8.c_str());
            return 1;
        }
        ...
        while (!specStream.eof()) {
            ...
            if (specStream.eof()) {
                ...
                ERR_PRINT("Found duplicate constant %u in %s.\n", constantId, specPathU8.c_str());
                return 1;
            }
            else {
        ...
        if (!respv::Optimizer::run(shader, specConstants.data(), specConstants.size(), optimizedData)) {
            ERR_PRINT("Failed to optimize SPIR-V data from %s.\n", spirvPathU8.c_str());
            return 1;
        }
        ...
        if (!outputStream.is_open()) {
            ERR_PRINT("Failed to open %s for writing.\n", optPathU8.c_str());
            return 1;
        }
        ...
        if (outputStream.bad()) {
            ...
            ERR_PRINT("Failed to write to %s.\n", optPathU8.c_str());
            return 1;
        }
        ...
        if (resultCode != 0) {
            ERR_PRINT("Failed to validate %s. Result code %d.\n", optPathU8.c_str(), resultCode);
            return 1;
        }
    }
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        ERR_PRINT("./re-spirv-cli <spirv-input-file> <spirv-output-file>");
        return 1;
    }
