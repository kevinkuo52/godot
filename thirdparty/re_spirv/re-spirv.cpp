...
namespace respv {
    ...
    bool Shader::parseWords(const void *pData, size_t pSize) {
        ...
        if (spirvWordCount < startingWordIndex) {
            ERR_PRINT("Not enough words in SPIR-V.\n");
            return false;
        }

        if (spirvWords[0] != SpvMagicNumber) {
            ERR_PRINT("Invalid SPIR-V Magic Number on header.\n");
            return false;
        }

        if (spirvWords[1] > SpvVersion) {
            ERR_PRINT("SPIR-V Version is too new for the library. Max version for the library is 0x%X.\n", SpvVersion);
            return false;
        }
        ...
        while (wordIndex < spirvWordCount) {
            ...
            if (hasResult) {
                ...
                if (resultId >= idBound) {
                    ERR_PRINT("SPIR-V Parsing error. Invalid Result ID: %u.\n", resultId);
                    return false;
                }
    ...
    bool Shader::process() {
        ...
        for (uint32_t i = 0; i < uint32_t(instructions.size()); i++) {
            ...
            if (!SpvIsSupported(opCode)) {
                ERR_PRINT("%s is not supported yet.\n", SpvOpToString(opCode));
				fprintf(stderr, "stderr %s is not supported yet.\n", SpvOpToString(opCode));
                return false;
            }
            ...
            if (hasType) {
                ...
                if (typeId >= results.size()) {
                    ERR_PRINT("SPIR-V Parsing error. Invalid Type ID: %u.\n", typeId);
                    return false;
                }

                if (results[typeId].instructionIndex == UINT32_MAX) {
                    ERR_PRINT("SPIR-V Parsing error. Result %u is not valid.\n", typeId);
                    return false;
                }
            ...
            if (SpvHasOperands(opCode, operandWordStart, operandWordCount, operandWordStride, operandWordSkip, operandWordSkipString)) {
                ...
                for (uint32_t j = 0; j < operandWordCount; j++) {
                    ...
                    if (operandId >= results.size()) {
                        ERR_PRINT("SPIR-V Parsing error. Invalid Operand ID: %u.\n", operandId);
                        return false;
                    }
            ...
            if (opCode == SpvOpPhi) {
                for (uint32_t j = 3; j < wordCount; j += 2) {
                    ...
                    if (labelId >= results.size()) {
                        ERR_PRINT("SPIR-V Parsing error. Invalid Parent ID: %u.\n", labelId);
                        return false;
                    }
        ...
        if (foundOpSwitch && (defaultSwitchOpConstantInt == UINT32_MAX)) {
            ERR_PRINT("Unable to find an OpConstantInt to use as replacement for switches. Adding this instruction automatically is not supported yet.\n");
            return false;
        }
        ...
        for (uint32_t i = 0; i < pNewSpecConstantCount; i++) {
            ...
            case SpvOpSpecConstant:
                if (constantWordCount <= 3) {
                    ERR_PRINT("Optimization error. Specialization constant has less words than expected.\n");
                    return false;
                }

                if (newSpecConstant.values.size() != (constantWordCount - 3)) {
                    ERR_PRINT("Optimization error. Value count for specialization constant %u differs from the expected size.\n", newSpecConstant.specId);
                    return false;
                }
                ...
                break;
            default:
				ERR_PRINT("Optimization error. Can't patch opCode %u.\n", constantOpCode);
                return false;
            }
    ...
    static bool optimizerCompactPhi(uint32_t pInstructionIndex, OptimizerContext &rContext) {
        ...
        if (instructionLabelId == UINT32_MAX) {
            ERR_PRINT("Unable to find a label before OpPhi.\n");
            return false;
        }
                ...
                if (SpvHasOperands(opCode, operandWordStart, operandWordCount, operandWordStride, operandWordSkip, operandWordSkipString)) {
                    ...
                    for (uint32_t j = 0; j < operandWordCount; j++) {
                        ...
                        if ((opCode != SpvOpPhi) && (rContext.resolutions[operandId].type == Resolution::Type::Unknown)) {
                            ERR_PRINT("Error in resolution of the operations. Operand %u was not solved.\n", operandId);
                            return false;
                        }
